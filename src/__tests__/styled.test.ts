...
import { createQrBadgeSvg } from "../qr/styled.js";
...
describe("createQrBadgeSvg", () => {
  ...
  it("can encode the bare payload", () => {
    const { content } = createQrBadgeSvg(PARTS, { encode: "payload" });
    expect(content).toBe(`1|${LT}|${CT}`);
  });

  it("renders one dot per dark non-finder module", () => {
    const { svg, content } = createQrBadgeSvg(PARTS);
  ...
  it("renders the branded frame with header by default", () => {
    const { svg, width, height } = createQrBadgeSvg(PARTS);
  ...
  it("omits frame and header when frame=false", () => {
    const { svg, width, height } = createQrBadgeSvg(PARTS, { frame: false });
    expect(svg).not.toContain("Secured by");
    expect(width).toBe(height);
  });

  it("escapes XML in custom header text", () => {
    const { svg } = createQrBadgeSvg(PARTS, { headerText: 'Powered by <"&>' });
    expect(svg).toContain("Powered by &lt;&quot;&amp;&gt;");
    expect(svg).not.toContain('by <"&>');
  });

  it("applies custom colours", () => {
    const { svg } = createQrBadgeSvg(PARTS, { colors: { navy: "#123456" } });
    expect(svg).toContain("#123456");
  });

  it("rejects unsafe colour attributes", () => {
    expect(() => createQrBadgeSvg(PARTS, { colors: { navy: '" onload="alert(1)' } })).toThrow(
      "colors.navy",
    );
    expect(() =>
      createQrBadgeSvg(PARTS, { colors: { panel: "url(javascript:alert(1))" } }),
    ).toThrow("colors.panel");
  });

  it("respects custom width", () => {
    const { svg, width } = createQrBadgeSvg(PARTS, { width: 720 });
    expect(width).toBe(720);
    expect(svg).toContain('width="720"');
  });

  it("rejects invalid widths", () => {
    expect(() => createQrBadgeSvg(PARTS, { width: 0 })).toThrow("width");
  });

  it("uses higher ECC levels on request", () => {
    const m = createQrBadgeSvg(PARTS, { errorCorrectionLevel: "M" });
    const h = createQrBadgeSvg(PARTS, { errorCorrectionLevel: "H" });
