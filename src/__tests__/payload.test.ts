...
const VERIFIABL_ID = "AbCdEfGhIjKlMnOpQrStUv"; // 22 base64url chars
const CIPHERTEXT = "Zm9vYmFyYmF6cXV4";

describe("buildBarcodePayload", () => {
  it("builds the v1 pipe format", () => {
    expect(buildBarcodePayload({ verifiablId: VERIFIABL_ID, encryptedPii: CIPHERTEXT })).toBe(
      `1|${VERIFIABL_ID}|${CIPHERTEXT}`,
    );
  });

  it("rejects Verifiabl IDs that are not 22 chars", () => {
    expect(() => buildBarcodePayload({ verifiablId: "short", encryptedPii: CIPHERTEXT })).toThrow();
  });

  it("rejects non-base64url ciphertext", () => {
    expect(() =>
      buildBarcodePayload({ verifiablId: VERIFIABL_ID, encryptedPii: "not+valid/" }),
    ).toThrow();
  });

  it("rejects empty ciphertext", () => {
    expect(() => buildBarcodePayload({ verifiablId: VERIFIABL_ID, encryptedPii: "" })).toThrow();
  });
});

describe("buildScanUrl", () => {
  it("wraps the payload in the production /v/ URL with URL encoding", () => {
    const url = buildScanUrl({ verifiablId: VERIFIABL_ID, encryptedPii: CIPHERTEXT });
    expect(url).toBe(
      `${DEFAULT_SCAN_BASE_URL}/v/${encodeURIComponent(`1|${VERIFIABL_ID}|${CIPHERTEXT}`)}`,
    );
  ...
  it("uses the sandbox scan URL when environment is sandbox", () => {
    const url = buildScanUrl(
      { verifiablId: VERIFIABL_ID, encryptedPii: CIPHERTEXT },
      { environment: "sandbox" },
    );
