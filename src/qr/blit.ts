...
const RADIUS_DENOM = 80;
const OUTER_RADIUS_80THS = 112;
const INNER_RADIUS_80THS = 80;
const DOT_RADIUS_80THS = 52;

// Q units per pixel-over-denom: lcm(S2, RADIUS_DENOM), so subsample centres
// and the corner radii are both integral in Q for every box size and width.
const Q_PER_PIXEL = 80;
const Q_PER_SUBSAMPLE_STEP = Q_PER_PIXEL / S2;
...
export function blitQrOntoFrame(frame: RgbaRaster, qr: QrBlitGeometry, pixelWidth: number): void {
  ...
  function renderFinder(raster: RgbaRaster, moduleX: number, moduleY: number): void {
    ...
    const outer = {
      ...
      radius: OUTER_RADIUS_80THS * radiusUnit,
    };
    const inner = {
      x0: outer.x0 + moduleQ,
      y0: outer.y0 + moduleQ,
      size: (FINDER_SIZE - 2) * moduleQ,
      radius: INNER_RADIUS_80THS * radiusUnit,
    };
    const dot = {
      x0: outer.x0 + 2 * moduleQ,
      y0: outer.y0 + 2 * moduleQ,
      size: (FINDER_SIZE - 4) * moduleQ,
      radius: DOT_RADIUS_80THS * radiusUnit,
    };
      ...
      for (let px = pxLo; px < pxHi; px++) {
        ...
        if (
          ...
          for (let sy = 0; sy < SUBSAMPLES; sy++) {
            const yQ = (py * Q_PER_PIXEL + (2 * sy + 1) * Q_PER_SUBSAMPLE_STEP) * denom;
            for (let sx = 0; sx < SUBSAMPLES; sx++) {
              if (blackAt((px * Q_PER_PIXEL + (2 * sx + 1) * Q_PER_SUBSAMPLE_STEP) * denom, yQ)) {
                count++;
              }
            }
          }
